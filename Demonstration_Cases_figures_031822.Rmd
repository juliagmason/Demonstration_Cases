...
ds_catch_nutr_yield_mt_baseline <- readRDS("Data/ds_catch_nutr_yield_baseline.Rds")
...
plot_rdas_met_scen <- function (country_name, imperfect = TRUE, anchovy = TRUE) {
  ...
  if (anchovy != TRUE) {
    ...
    title = paste0("Percent of population RDAs met under future management scenarios, ", country_name, ",\nanchovy removed")
  } else {
...
Nutricast used nutrient supply from the GENuS database, Estimated Average Requirements (EARs), which represent the nutrients needed to meet the needs of half the population, and sex/age specific estimated population size/growth to calculate micronutrient deficiencies. They used an optimization function to expand the EARs to instead reach 95% of the nutritional needs of the population, but it seems that this calculation calls for much higher values than the Recommended Dietary Allowance (RDA) values, which are supposed to meet the needs of 97-98% of the population. I have access to this code so can get age/sex specific deficiency values. 

Beal et al. (2017) calculate a Prevalence of Inadequate Micronutrient Intake Index (PIMII) using I believe the same method as above (nutricast probably modeled after them), based on EAR cutpoints, but gets different values than Nutricast. They also provide estimates accounting for fortification in diets. These data are from 2011 and do not include selenium or Omega 3s. Hicks and co (Maire, Robinson) use these values, which are available as overall index and by individual nutrients from Beal's supplemental data. I therefore only have population-level values, not age/sex specific. 

Chris Golden's AFCD/Blue Foods Assessment paper used the proprietary Global Nutrient Database (based on FAO Supply and Utilization Accounts) for national nutrient availability, and GENuS for intake/consumption. They used a habitual intake statistical program to calculate consumption of Omega 3s and vitamin B12, which are not in GENuS. They used SEVs (summary exposure values) as a risk-weighted prevalence of inadequate micronutrient intake. I'm not sure yet what the benefit is of this method over the others. 

With the Nutricast and Beal data, I have deficiencies for Calcium, Vitamin A, Zinc, and Iron (these are also what Hicks et al looked at). For all our countries, calcium has the highest prevalence of deficiency, followed by Vitamin A for every country but Peru (Zinc, then Vitamin A). However, since there are only four nutrients to look at, and because these data don't necessarily reflect what we've found in our country-specific research (e.g. concerns about anemia in Peru and iron and zinc deficiencies in Chile), we might as well look at all of them.

See "Micronutrient deficiences" table in google doc for prevalence of inadequate intake values from these two sources. 
...
In the previous set of figures, I showed percent of nutrient demand met under the various scenarios using the Nutricast method. Here instead I'm using RDAs. We get somewhat bigger numbers because the RDA values are smaller than the Nutricast optimization methodm but not a huge difference. 

```{r SL_future_rdas, echo = FALSE}

plot_rdas_met_scen("Sierra Leone")

```

```{r In_future_rdas, echo = FALSE}

plot_rdas_met_scen("Indonesia", imperfect = FALSE)

```

```{r chile_future_rdas, echo = FALSE}

plot_rdas_met_scen("Chile", imperfect = FALSE)

```

```{r chile_future_rdas_noanchov, echo = FALSE}

plot_rdas_met_scen("Chile", imperfect = FALSE, anchovy = FALSE)

```

```{r Peru_future_rdas, echo = FALSE}
