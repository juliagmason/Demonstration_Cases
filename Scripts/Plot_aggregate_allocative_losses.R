...
# mean of most recent 5 years for exports 
...
# FIX this with fleet, can probably use case_when
plot_trade_levers_aggregate <- function (country_name, Selenium = FALSE) {
   ...
   z <- sau_2019 %>%
    ...
    left_join (exports_5yr_mean, by = c("species", "country")) %>%
     ...
     mutate (Exported = ifelse (fleet == "Domestic catch", tonnes * mn_prop_exp, 0),
             Domestic_catch = ifelse (fleet == "Domestic catch", tonnes * (1-mn_prop_exp), 0),
             Foreign_catch = ifelse (fleet == "Foreign catch", tonnes, 0),
             .keep = "unused") %>%
    # pivot longer
  ...
  w <- z %>%
    ...
    ggplot (aes (y = rni_equivalents/1000000, x = reorder(nutrient, -rni_equivalents), fill = lever)) +
    geom_col () +
    theme_bw()+
...
c %>%
  ...
  summarise (rni_equivalents = sum (children_fed, na.rm = TRUE)) %>%
...
x_nutr <- x %>%
  mutate (rni_equivalents = pmap (list (species = species, amount = catch_end_use, country_name = "Chile"), calc_children_fed_func)) %>%
  unnest (cols = c(rni_equivalents)) %>%
...
x_nutr %>%
  group_by (nutrient, end_use_type) %>%
  summarise (rni_equivalents = sum (rni_equivalents, na.rm = TRUE)) %>%
  ggplot (aes (x = reorder(nutrient, -rni_equivalents), y = rni_equivalents/1000000, fill = end_use_type)) +
  geom_col () +
  theme_bw()+
...
z %>%  
  ...
  group_by (nutrient, lever) %>%
  summarise (rni_equivalents = sum (rni_equivalents, na.rm = TRUE)) %>%
  ggplot (aes (x = reorder(nutrient, -rni_equivalents), y = rni_equivalents/1000000, fill = lever)) +
  geom_col () +
  theme_bw()+
